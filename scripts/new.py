...
class Screen:
    def __init__(self, num_start, ref_pic, is_changing_enabled):
        im = Image.open(ref_pic)
        ...
        self.id_num_start = num_start
        self.is_changing_enabled = is_changing_enabled

    def append_bits(x, bits):
        bits = bits.append([int(i) for i in bin(x)[2:]])

    def send_can_msg(self):
        id_num = self.id_num_start
        bus = can.Bus(interface='socketcan',channel='can0',bitrate=500000,receive_own_messages=False)
        frame_counter = 0
